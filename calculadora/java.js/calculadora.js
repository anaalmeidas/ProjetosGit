onload = () => {
  ...
  document.querySelector('#bt-Pliquido').onclick = () => operador('PL');
...
const calcula = () => {
  if (operacaoPendente != null) {
    ...
    switch (operacaoPendente) {
      case '+':
        resultado = valorAnterior + valorAtual(); break;
      case '-':
        resultado = valorAnterior - valorAtual(); break;
      case '*':
        resultado = valorAnterior * valorAtual(); break;
      case '/':
        resultado = valorAnterior / valorAtual(); break;
      case 'PL':
        if (valorAnterior <= 300) {
          bruto = 500 + (0.7 * valorAnterior);
          imposto = bruto * 0.25;
          resultado = bruto - imposto;
        } else if (valorAnterior <= 1000) {
          bruto = 200 + (0.5 * valorAnterior);
          imposto = bruto * 0.25;
          resultado = bruto - imposto;
        } else if (valorAnterior > 1000) {
          bruto = (0.3 * valorAnterior);
          imposto = bruto * 0.25;
          resultado = bruto - imposto;
        }
    }
